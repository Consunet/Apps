...
const testVars = require("./mocha_test_vars")
...
describe('WhisperNote Testing', function () {
    ...
    before(async function () {
        ...
        if( isCollectCoverage ) {
            coverageServer.startCoverageServer();
        }
        ...
        let options = new firefox.Options()
            ...
            .setProfile(comsupport.commonFullPath() + "/firefox_profile/")
            .headless();
    ...
    it('Can verify basic app details.', async function () {
        ...
        assert.equal(title, 'WhisperNote Encrypted Note v2.0', 'WhisperNote homepage title is NOT the one expected');
    ...
    describe('File encrypt and decrypt tests', function () {
        ...
        it('Can create a note without an attachment, then encrypt it.', async function () {
            ...
            expect(file('./src/tests/test_downloads/test_encrypted.html')).to.exist;

            //note should be deleted on encryption
            await support.assertNoteText(driver, '');

            //copy to web server path to access via URL
            fs.copyFileSync('./src/tests/test_downloads/test_encrypted.html', './public_html/test_encrypted.html');
        });
        ...
        it('Can create a note with an attachment, then encrypt it.', async function () {
            ...
            await driver.findElement(webdriver.By.id('file')).sendKeys(path.normalize(process.cwd() + '/src/tests/testmessage.txt'));
            ...
            //make sure file is created
            expect(file('./src/tests/test_downloads/test_encrypted_attachment.html')).to.exist;
            ...
            //copy to web server path to access via URL
            fs.copyFileSync('./src/tests/test_downloads/test_encrypted_attachment.html', './public_html/test_encrypted_attachment.html');
        });
        ...
        it('Can do decrypt of encrypted file via URL (with attachment).', async function () {
            ...
            expect(file('./src/tests/test_downloads/testmessage.txt')).to.exist;

            expect(file('./src/tests/test_downloads/testmessage.txt')).to.equal(file("./src/tests/testmessage.txt"));
        });
        ...
        it('Can do decrypt of encrypted file via import (with attachment).', async function () {
            ...
            fs.unlinkSync('./src/tests/test_downloads/testmessage.txt');
